...
int main() {
  ...
  ASSERT(status_code("208 Already Reported") ==
         StatusCode::success_already_reported);
  ASSERT(status_code(StatusCode::success_already_reported) ==
         "208 Already Reported");
  ASSERT(status_code("308 Permanent Redirect") ==
         StatusCode::redirection_permanent_redirect);
  ASSERT(status_code(StatusCode::redirection_permanent_redirect) ==
  ...
  ASSERT(status_code("502 Bad Gateway") ==
         StatusCode::server_error_bad_gateway);
  ASSERT(status_code(StatusCode::server_error_bad_gateway) ==
         "502 Bad Gateway");
  ASSERT(status_code("504 Gateway Timeout") ==
         StatusCode::server_error_gateway_timeout);
  ASSERT(status_code(StatusCode::server_error_gateway_timeout) ==
         "504 Gateway Timeout");
  ASSERT(status_code("511 Network Authentication Required") ==
         StatusCode::server_error_network_authentication_required);
  ASSERT(
      status_code(StatusCode::server_error_network_authentication_required) ==
