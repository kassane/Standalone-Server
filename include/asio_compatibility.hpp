...
inline void restart(io_context &context) noexcept { context.restart(); }
...
inline std::unique_ptr<asio::steady_timer> make_steady_timer(
    socket_type &socket, std::chrono::duration<duration_type> duration) {
...
inline void async_resolve(asio::ip::tcp::resolver &resolver,
  ...
  resolver.async_resolve(host_port.first, host_port.second,
...
inline void restart(io_context &context) noexcept { context.reset(); }
inline asio::ip::address make_address(const std::string &str) noexcept {
...
template <typename handler_type>
inline asio::detail::wrapped_handler<strand, handler_type,
