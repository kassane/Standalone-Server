 ...
 public:
  ...
  static CaseInsensitiveMultimap parse(
    ...
    if (name_pos < query_string.size()) {
      auto name =
          query_string.substr(name_pos, (name_end_pos == std::string::npos
                                             ? std::string::npos
      ...
      static CaseInsensitiveMultimap parse(const std::string &value) {
        ...
        for (std::size_t c = 0; c < value.size(); ++c) {
          if (name_start_pos == std::string::npos) {
            if (value[c] != ' ' && value[c] != ';') name_start_pos = c;
          } else {
            if (name_end_pos == std::string::npos) {
              if (value[c] == ';') {
                result.emplace(value.substr(name_start_pos, c - name_start_pos),
                               std::string());
              ...
              if (value_start_pos == std::string::npos) {
                ...
                result.emplace(
                    ...
                    Percent::decode(
                        value.substr(value_start_pos, c - value_start_pos)));
                name_start_pos = std::string::npos;
                name_end_pos = std::string::npos;
          ...
          if (name_end_pos == std::string::npos)
            ...
            if (value.back() == '"')
              result.emplace(
                  value.substr(name_start_pos, name_end_pos - name_start_pos),
                  Percent::decode(
                      value.substr(value_start_pos, value.size() - 1)));
            else
              result.emplace(
                  value.substr(name_start_pos, name_end_pos - name_start_pos),
  ...
  static bool parse(std::istream &stream, std::string &version,
    ...
    if (getline(stream, line) &&
      ...
      if ((version_end + 1) < line.size())
        status_code =
            line.substr(version_end + 1, line.size() - (version_end + 1) -
                                             (line.back() == '\r' ? 1 : 0));
      else
        return false;
