...
public:
  ...
  std::pair<asio::buffers_iterator<asio::const_buffer>, bool>
  operator()(asio::buffers_iterator<asio::const_buffer> begin,
             asio::buffers_iterator<asio::const_buffer> end) {
    auto it = begin;
    for (; it != end; ++it) {
      if (*it == '\n') {
        if (crlfcrlf == 1)
...
template <class socket_type> class ClientBase {
public:
  ...
  public:
    ...
    /// When true, this is the last response content part from server for the
    /// current request.
    bool end = true;
...
public:
  class Response {
    ...
    Response(std::size_t max_response_streambuf_size,
        ...
        : streambuf(max_response_streambuf_size), shared(new Shared()),
          connection_weak(connection_), http_version(shared->http_version),
          status_code(shared->status_code), header(shared->header),
          content(streambuf) {}

    /// Constructs a response object that has empty content, but otherwise is
    /// equal to the response parameter
    Response(const Response &response) noexcept
        : streambuf(response.streambuf.max_size()), shared(response.shared),
          connection_weak(response.connection_weak),
          http_version(shared->http_version), status_code(shared->status_code),
          header(shared->header), content(streambuf) {}
...
public:
  ...
  void request(const std::string &method, const std::string &path,
    ...
    session->callback = [this, session_weak,
                         request_callback](const error_code &ec) {
...
protected:
  ...
  std::unordered_set<std::shared_ptr<Connection>>
      connections GUARDED_BY(connections_mutex);
  ...
  std::shared_ptr<Response>
  sync_request(const std::string &method, const std::string &path,
               ContentType &content, const CaseInsensitiveMultimap &header) {
    {
  ...
  std::shared_ptr<Connection> get_connection() noexcept {
    ...
    if (!host_port) {
      if (config.proxy_server.empty())
        host_port = std::unique_ptr<std::pair<std::string, std::string>>(
            new std::pair<std::string, std::string>(host,
                                                    std::to_string(port)));
      else {
  ...
  parse_host_port(const std::string &host_port,
  ...
  std::unique_ptr<asio::streambuf>
  create_request_header(const std::string &method, const std::string &path,
        ...
        [this, session](const error_code &ec, std::size_t bytes_transferred) {
          ...
          if (!ec) {
            ...
            if (header_it != session->response->header.end()) {
              ...
              session->callback(
                  ec); // Connection to a Server-Sent Events resource is opened
                  ...
                  *session->connection->socket, session->response->streambuf,
                  ...
                  [this, session, chunk_size_streambuf](
                    ...
                    if (!ec) {
                      ...
                      asio::async_read(
                          *session->connection->socket, *null_buffer,
...
template <class socket_type> class Client : public ClientBase<socket_type> {};

using HTTP = asio::ip::tcp::socket;

template <> class Client<HTTP> : public ClientBase<HTTP> {
public:
  /**
          ...
          *resolver, *host_port,
          [this, session, resolver](const error_code &ec,
            ...
            if (!ec) {
              session->connection->set_timeout(config.timeout_connect);
              asio::async_connect(
                  *session->connection->socket, results,
                  [this, session,
                   resolver](const error_code &ec,
