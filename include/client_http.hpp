...
class ClientBase {
 ...
 public:
  class Response {
   ...
   public:
    ...
    /// parts from server.
    void close() noexcept {
      if (auto connection = this->connection_weak.lock()) connection->close();
    }
  };
  ...
  void request(const std::string &method, const std::string &path,
    ...
    session->callback = [this, session_weak,
      ...
      if (auto session = session_weak.lock()) {
        ...
        if (*request_callback) (*request_callback)(session->response, ec);
      }
    };
 ...
 protected:
  ...
  std::shared_ptr<Response> sync_request(
      const std::string &method, const std::string &path, ContentType &content,
    ...
    request(method, path, content, header,
            [&response, &response_promise, stop_future_handlers](
              ...
              if (*stop_future_handlers) return;
  ...
  std::unique_ptr<asio::streambuf> create_request_header(
    ...
    if (port != default_port) write_stream << ':' << std::to_string(port);
    write_stream << "\r\n";
    for (auto &h : header)
