...
inline double round(
    double x) noexcept {  // Custom definition of round() for positive numbers
  return floor(x + 0.5);
}
#endif
...
// TODO: Remove in 2022 or later
  ...
  static std::string message_digest(const std::string &str,
                                    const EVP_MD *evp_md,
  ...
  static std::string stream_digest(std::istream &stream, const EVP_MD *evp_md,
                                   std::size_t digest_length) noexcept {
    std::string md(digest_length, '\0');
    ...
    while ((read_length = stream.read(buffer.get(), buffer_size).gcount()) > 0)
      EVP_DigestUpdate(ctx, buffer.get(),
