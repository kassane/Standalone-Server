...
class ServerBase {
  ...
  class Response : public std::enable_shared_from_this<Response>,
      ...
      for (auto &field : header) {
        if (!content_length_written &&
          ...
          chunked_transfer_encoding = true;
                 ...
                 case_insensitive_equal(field.second, "text/event-stream"))
          event_stream = true;
    ...
    void send_from_queue() REQUIRES(send_queue_mutex) {
      ...
      post(session->connection->write_strand, [self, buffer] {
        ...
        if (!lock) return;
        asio::async_write(
            *self->session->connection->socket, buffer,
            [self](const error_code &ec, std::size_t /*bytes_transferred*/) {
              ...
              if (!lock) return;
              {
                ...
                if (!ec) {
                  ...
                  if (self->send_queue.size() > 0) self->send_from_queue();
                  ...
                  lock.unlock();
                  for (auto &callback : callbacks) callback(ec);
                }
              }
            });
      });
   ...
   public:
    ...
    void send(
      ...
      if (send_queue.size() == 1) send_from_queue();
    }
    ...
    /// content.
    void write(
        StatusCode status_code, string_view content,
        const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
      *this << "HTTP/1.1 " << SimpleWeb::status_code(status_code) << "\r\n";
 ...
 protected:
  class Connection : public std::enable_shared_from_this<Connection> {
   public:
    ...
    /**
     * Needed for TLS communication where async_write could be called outside of
     * the io_context runners. For more information see
    ...
    void set_timeout(long seconds) noexcept {
      ...
      timer->async_wait([self_weak](const error_code &ec) {
        if (!ec) {
          if (auto self = self_weak.lock()) self->close();
        }
      });
    }
  ...
  class regex_orderable : public regex::regex {
   public:
    ...
    regex_orderable(std::string regex_str_)
        : regex::regex(regex_str_), str(std::move(regex_str_)) {}
 ...
 public:
  ...
  void start(
    ...
    if (!config.address.empty())
      endpoint =
          asio::ip::tcp::endpoint(make_address(config.address), config.port);
    else
    ...
    if (internal_io_service && io_service->stopped()) restart(*io_service);

    if (callback) post(*io_service, [callback, port] { callback(port); });
  ...
  void stop() noexcept {
    ...
    if (acceptor) {
...

      {
        ...
        for (auto &connection : connections->set) connection->close();
 ...
 protected:
        ...
        [connections](Connection *connection) {
          {
            ...
            if (it != connections->set.end()) connections->set.erase(it);
          }
          delete connection;
        });
    {
                    ...
                    if (!ec) {
                      ...
                      this->on_error(session->request, ec);
                       ...
                       num_additional_bytes) {  // If only end of chunk remains
                                                // unread (\n or \r\n)
              // Remove "\r\n"
              if (2 + chunk_size - num_additional_bytes == 1) istream.get();
              auto null_buffer = std::make_shared<asio::streambuf>(2);
              asio::async_read(
                  *session->connection->socket, *null_buffer,
  ...
  void find_resource(const std::shared_ptr<Session> &session) {
    ...
    if (it != default_resource.end()) write(session, it->second);
  }
  ...
  void accept() override {
    ...
    acceptor->async_accept(
        *connection->socket, [this, connection](const error_code &ec) {
          ...
          // been stopped)
          if (ec != error::operation_aborted) this->accept();
